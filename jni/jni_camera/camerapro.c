...
void JNICALL Java_com_jni_bitmap_Bmp_huidu(JNIEnv *env, jclass clazz, jobject zBitmap) {
    JNIEnv J = *env;
        ...
        for (x = 0; x < info.width; ++x) {
            ...
            if (info.format == ANDROID_BITMAP_FORMAT_RGB_565) {
                *((uint16_t *)pixel) = MAKE_RGB565(gray, gray, gray);
                ...
                // *((uint32_t *)pixel) = MAKE_RGBA(gray, gray, gray, a); // 灰度处理
            	{ // 二值化处理
            		uint32_t v = MAKE_RGBA(r, g, b, a);
            		*((uint32_t *)pixel) = v;
            	}
            }
        }
